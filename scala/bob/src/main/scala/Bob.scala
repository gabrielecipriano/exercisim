object Bob {
  ...
  def response(statement: String): String = statement match {
    ...
    case s if isAskingCalmly(s) => "Sure."
    case s if isShouting(s) => "Whoa, chill out!"
    case s if isSilent(s) => "Fine. Be that way!"
    case _ => "Whatever."
  }

  private def isAskingShouting(s: String) = isShouting(s) && isAskingCalmly(s)

  private def isAskingCalmly(statement: String): Boolean = statement.trim.endsWith("?")
